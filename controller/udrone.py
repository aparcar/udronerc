...
class DroneGroup(object):
    ...
    def request(self, type, data=None, timeout=60):
        ...
        while len(pending) > 0 and (now - start) >= 0 and (now - start) < timeout:
            ...
            if i % 2 == 1:
                answers.update(
                    self.host.call(self.groupid, seq, type, data, expect=expect)
                )
            else:
    ...
    def __init__(self, local=None, args=[]):
        ...
        if local:
            self.socket.setsockopt(
                socket.SOL_IP, socket.IP_MULTICAST_IF, socket.inet_aton(local)
            )
...
if __name__ == "__main__":
    ...
    if len(source) < 1:
        print(
            "!!! WARNING: You haven't set a source address !!!",
